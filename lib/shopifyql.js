...
export async function runShopifyQL(store, query, opts = {}) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    // `rows` comes back either as arrays of values aligned to `columns`, or as
    // objects already keyed by column name, depending on API version. Normalise
    // both to column-keyed objects.
    ...
    const rows = rawRows.map((row) => {
      ...
      // Scalar row (single-column result)
      return columns.length ? { [columns[0]]: row } : { value: row };
    });
