...
async function dbSlice(store, from, to, granularity) {
  const cov = await coverage(store.key, from, to);
  if (!cov.days) return null;
  ...
  if (staleness > MAX_STALENESS_DAYS) return null;

  const rows = await readDailyMetrics(store.key, from, to);
...
export async function fetchStoreDataServed(store, range, compare, opts = {}) {
    ...
    return {
      ...
      warnings: [
        ...live.warnings,
        {
          ...
          code: "NOT_SYNCED",
          message:
            "This range isn't in the synced database yet, so it was fetched live from Shopify. Run /api/sync to backfill it.",
        },
      ],
    };
  }
  ...
  if (compare?.from && !cmp) {
    warnings.push({
      ...
      dataset: "comparison",
      code: "NOT_SYNCED",
      message: "The comparison range isn't synced yet — comparison figures will read as zero until it is.",
    });
  }
