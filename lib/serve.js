...
async function dbSlice(store, from, to, granularity) {
  ...
  // The sync stores a row per day Shopify reported, so gaps inside a range are
  // ordinary zero-activity days. A range that *starts* before the first synced
  // day is different: those weeks are missing, not empty, and a year-on-year
  // comparison built on them would quietly understate the earlier period.
  const partial = cov.minDay && cov.minDay > from ? cov.minDay : null;

  return { agg: aggregateRows(rows, granularity), cov, partial };
}
...
export async function fetchStoreDataServed(store, range, compare, opts = {}) {
  ...
  for (const [dataset, slice, start] of [
    ["range", primary, range.from],
    ["comparison", cmp, compare?.from],
  ]) {
    if (!slice?.partial) continue;
    warnings.push({
      store: store.key,
      dataset,
      code: "PARTIAL_SYNC",
      message:
        `Synced data for this store starts on ${slice.partial}, after the requested ${start}. ` +
        `The ${dataset === "comparison" ? "comparison" : "period"} total covers only part of the window — ` +
        "run /api/sync with a longer ?days= to fill it in.",
    });
  }
