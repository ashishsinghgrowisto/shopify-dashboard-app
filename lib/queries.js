...
// Grouping sales by a UTM dimension requires an explicit attribution model —
// Shopify rejects the query outright with "Attribution Syntax Required" otherwise.
// Last non-direct click matches what the old CSV export used.
// When an attribution model is applied, the plain metric names are rejected —
// Shopify requires the attributed variants (metric__last_non_direct_click).
export function utmFallbackSalesQuery(from, to, limit = 60) {
  return [
    "FROM sales",
    // Only metrics that support attribution may appear here. total_sales is the
    // one Shopify names explicitly; orders/AOV have no attributed variant, so
    // order counts come from the sessions query and AOV is derived downstream.
    "SHOW total_sales__last_non_direct_click",
    "GROUP BY utm_campaign",
    "WITH LAST_NON_DIRECT_CLICK_ATTRIBUTION",
    `SINCE ${from} UNTIL ${to}`,
    "ORDER BY total_sales__last_non_direct_click DESC",
    `LIMIT ${limit}`,
  ].join(" ");
}
...
export async function fetchStoreData(store, range, compare, opts = {}) {
  ...
  // Campaign fallback: the campaign_* marketing schemas are frequently either
  // unavailable or simply empty on a store (they return ok with zero rows and no
  // columns). Either way there's nothing to render, so fall back to the utm
  // dimensions on sessions/sales, which are populated far more reliably.
  const campaignsUnusable = (r) => r.failed || (r.rows || []).length === 0;

  if (campaignsUnusable(byLabel.campaignSessions) && campaignsUnusable(byLabel.campaignSales)) {
...
// Public reshapers
// ══════════════════════════════════════════════════════════════════
// Exposed for lib/serve.js, which fetches these two breakdowns on demand while
// the timeseries half is served from Postgres.

export function reshapeReferrersPublic(result) {
  if (!result || result.failed) return [];
  return reshapeReferrers(result);
}

export function reshapeCampaignsPublic(sessionsRes, salesRes) {
  return reshapeCampaigns(sessionsRes, salesRes);
}
