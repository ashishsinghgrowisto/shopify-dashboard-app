...
// Every secret this deployment knows about, for verifying inbound webhooks.
//
// Webhooks don't carry a slot: Shopify signs the body with the secret of
// whichever app sent it, and a deployment serving one public app plus several
// legacy custom apps has to accept all of them. Returning the set lets the
// webhook route try each in constant time rather than guess.
export function allAppSecrets() {
  const secrets = new Set();
  for (let s = 1; s <= MAX_SLOT; s++) {
    const { apiSecret, configured } = getAppCredentials(s);
    if (configured && apiSecret) secrets.add(apiSecret);
  }
  return [...secrets];
}
